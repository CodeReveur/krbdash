...
export async function POST(req: Request) {
  try {
    ...
    const queryTotalUploads = `SELECT COUNT(*) AS total_uploads FROM researches WHERE user_id = $1;`;
    ...
    const query = `
      ...
      FROM researches r
      WHERE r.user_id = $1 AND EXTRACT(MONTH FROM CAST(r.created_at AS DATE)) = $2 
